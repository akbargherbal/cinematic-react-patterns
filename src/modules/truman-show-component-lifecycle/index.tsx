...
Good afternoon, good evening, and good night.`,
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-sans text-slate-300">
      {/* Header */}
      <header className="border-b border-sky-500/20 bg-gradient-to-b from-slate-900 to-slate-950 p-8">
        <div className="mx-auto max-w-6xl">
          <h1 className="mb-2 text-5xl font-bold text-sky-400">
            The Truman Show
          </h1>
          <p className="mb-4 text-xl text-slate-400">Seahaven Island, 1998</p>
      ...
      <main className="mx-auto max-w-6xl px-4 py-12 pb-32">
        {/* Chapter Title */}
        <div className="mb-8">
          <h2 className="mb-2 text-3xl font-bold text-sky-300">
            {currentChapter.title}
          </h2>
          <div className="h-1 w-24 rounded-full bg-gradient-to-r from-sky-500 to-cyan-500"></div>
        </div>
      ...
      {/* Chapter Navigation */}
      <footer className="fixed bottom-0 left-0 right-0 border-t border-sky-500/20 bg-slate-900/95 p-4 backdrop-blur-sm">
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          <button
            onClick={handlePrevious}
            disabled={chapter === 0}
            className="rounded-lg bg-sky-600 px-6 py-2 font-semibold text-white transition-colors hover:bg-sky-500 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-500"
...
function ControlRoomDemo() {
  ...
  useEffect(() => {
    ...
    timers.push(
      setTimeout(() => setSystems((s) => ({ ...s, sunrise: true })), 300),
    );
    timers.push(
      setTimeout(() => setSystems((s) => ({ ...s, weather: true })), 600),
    );
    timers.push(
      setTimeout(() => setSystems((s) => ({ ...s, actors: true })), 900),
    );
    timers.push(
      setTimeout(() => setSystems((s) => ({ ...s, cameras: true })), 1200),
    );
  ...
  return (
    <div className="rounded-lg border border-sky-500/30 bg-slate-900/50 p-6">
      <h3 className="mb-4 flex items-center gap-2 text-2xl font-bold text-sky-300">
        <Camera className="h-6 w-6" />
        The Control Room
      </h3>
      <p className="mb-6 text-slate-400">
        Watch the useEffect setup function execute when Truman's component
        mounts. Each system activates in sequence.
      </p>

      <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2">
        <SystemIndicator
          icon={Sun}
          label="Sunrise System"
          active={systems.sunrise}
        />
        <SystemIndicator
          icon={Cloud}
          label="Weather System"
          active={systems.weather}
        />
        <SystemIndicator
          icon={Users}
          label="Actor Deployment"
          active={systems.actors}
        />
        <SystemIndicator
          icon={Camera}
          label="Surveillance (5000 cameras)"
          active={systems.cameras}
        />
      </div>
      ...
      <div className="flex gap-4">
        <button
          onClick={handleMount}
          disabled={mounted}
          className="flex items-center gap-2 rounded-lg bg-sky-600 px-6 py-3 font-semibold text-white transition-colors hover:bg-sky-500 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-500"
        >
        ...
        <button
          ...
          disabled={!mounted}
          className="flex items-center gap-2 rounded-lg bg-cyan-600 px-6 py-3 font-semibold text-white transition-colors hover:bg-cyan-500 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-500"
        >
      ...
      <div className="mt-6 rounded border border-slate-700 bg-slate-950/50 p-4 font-mono text-sm">
        <div className="mb-2 text-cyan-400">useEffect(() =&gt; &#123;</div>
        <div className="pl-4 text-slate-400">
          <div className={systems.sunrise ? "text-sky-300" : ""}>
            const sunrise = scheduleSunrise(7, 0);
          </div>
          <div className={systems.weather ? "text-sky-300" : ""}>
            const weather = initializeWeatherSystem();
          </div>
          <div className={systems.actors ? "text-sky-300" : ""}>
            const actors = deployActors(truman.location);
          </div>
          <div className={systems.cameras ? "text-sky-300" : ""}>
            const surveillance = activateCameras(5000);
          </div>
        </div>
        <div className="mt-2 text-cyan-400">&#125;, [truman.mounted]);</div>
      </div>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <Icon
          className={`h-6 w-6 ${active ? "animate-pulse text-sky-400" : "text-slate-600"}`}
        />
        <span
          className={active ? "font-semibold text-sky-300" : "text-slate-500"}
        >
          {label}
        </span>
      </div>
...
function DependencyTrackerDemo() {
  ...
  return (
    <div className="rounded-lg border border-sky-500/30 bg-slate-900/50 p-6">
      ...
      <p className="mb-6 text-slate-400">
        Change Truman's state and watch which effects re-run. Notice how the
        effect tracks both dependencies.
      </p>

      <div className="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2">
        <div>
          <label className="mb-2 block font-semibold text-sky-300">
            Truman's Location
          </label>
          <select
            value={location}
            onChange={(e) => setLocation(e.target.value)}
            className="w-full rounded-lg border border-slate-700 bg-slate-800 px-4 py-2 text-slate-300 focus:border-sky-500 focus:outline-none"
          >
      ...
      <div className="mt-4 rounded border border-cyan-500/30 bg-slate-800/50 p-4">
        <div className="mb-2 font-semibold text-cyan-400">Key Insight:</div>
        <p className="text-sm text-slate-300">
          The effect re-runs whenever{" "}
          <code className="text-sky-400">location</code> or{" "}
          <code className="text-sky-400">suspicion</code> changes. If we forgot
          to include <code className="text-sky-400">suspicion</code> in the
          dependency array, the effect would use stale values—just like the
          production crew not noticing Truman's growing awareness.
        </p>
      </div>
    </div>
  );
...
function CleanupComparisonDemo() {
  ...
  return (
    <div className="rounded-lg border border-sky-500/30 bg-slate-900/50 p-6">
      ...
      <p className="mb-6 text-slate-400">
        Compare what happens when a component unmounts with and without cleanup
        functions. Without cleanup, resources leak—timers keep running, creating
        chaos.
      </p>
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        >
          <h4 className="mb-2 font-bold text-sky-300">With Cleanup</h4>
          <p className="mb-4 text-sm text-slate-400">
            Effect runs, then cleanup executes on unmount. Resources freed
            cleanly.
          </p>
          {mounted && withCleanup && (
            <div className="animate-pulse text-sm text-sky-400">
              ✓ Running smoothly
            </div>
          )}
        </div>
      ...
      <div className="mb-6 flex gap-4">
        ...
        <button
          onClick={handleReset}
          className="flex items-center gap-2 rounded-lg bg-slate-700 px-6 py-3 font-semibold text-white transition-colors hover:bg-slate-600"
        >
      ...
      <div className="rounded border border-slate-700 bg-slate-950/50 p-4 font-mono text-sm">
        <div className="mb-2 text-cyan-400">useEffect(() =&gt; &#123;</div>
        <div className="pl-4 text-slate-400">
        ...
        <div className="mt-2 pl-2 text-cyan-400">
          {withCleanup ? (
            <>
              <div>return () =&gt; &#123;</div>
...
function UnmountSequenceDemo() {
  ...
  const steps = [
    {
      ...
      systems: { lights: true, actors: true, cameras: true, weather: true },
    },
    {
      label: "Unmount Triggered",
      systems: { lights: true, actors: true, cameras: true, weather: true },
    },
    {
      label: "Cleanup: Lights Dimming",
      systems: { lights: false, actors: true, cameras: true, weather: true },
    },
    {
      label: "Cleanup: Actors Leaving",
      systems: { lights: false, actors: false, cameras: true, weather: true },
    },
    {
      label: "Cleanup: Cameras Shutting Down",
      systems: { lights: false, actors: false, cameras: false, weather: true },
    },
    {
      label: "Cleanup: Weather System Off",
      systems: { lights: false, actors: false, cameras: false, weather: false },
    },
    {
  ...
  return (
    <div className="rounded-lg border border-sky-500/30 bg-slate-900/50 p-6">
      ...
      <p className="mb-6 text-slate-400">
        Step through the unmounting process. Watch cleanup functions execute in
        order, freeing resources gracefully.
      </p>

      <div className="mb-6 rounded-lg border border-cyan-500/30 bg-slate-800/50 p-4">
      ...
      {step === steps.length - 1 && (
        <div className="mt-6 rounded-lg border border-sky-500 bg-sky-500/10 p-4">
          <div className="mb-2 font-semibold text-sky-300">
            ✓ Clean Unmount Complete
          </div>
          <p className="text-sm text-slate-300">
            All resources freed. All cleanup functions executed. The component
            has unmounted gracefully, just like Truman walking through the door.
          </p>
        </div>
      )}
    </div>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div
          className={`h-3 w-3 rounded-full ${active ? "animate-pulse bg-sky-400" : "bg-slate-600"}`}
        />
      </div>
    </div>
  );
...
function LifecycleTimelineDemo() {
  const [phase, setPhase] = useState<
    "unmounted" | "mounting" | "mounted" | "updating" | "unmounting"
  >("unmounted");
  ...
  return (
    <div className="rounded-lg border border-sky-500/30 bg-slate-900/50 p-6">
      ...
      <p className="mb-6 text-slate-400">
        Visualize the full component lifecycle: mount → update → unmount. See
        when effects run and cleanup executes.
      </p>

      <div className="mb-8">
        <div className="mb-4 flex items-center justify-between">
          <PhaseIndicator label="Unmounted" active={phase === "unmounted"} />
          <div className="mx-2 h-1 flex-1 bg-slate-700" />
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        >
          <div className="mb-2 font-bold text-sky-300">Setup Function</div>
          <div className="text-sm text-slate-400">
            Runs on mount and when dependencies change
          </div>
          {(phase === "mounting" || phase === "updating") && (
            <div className="mt-2 animate-pulse text-xs text-sky-400">
              → Executing...
            </div>
          )}
        </div>
        ...
        >
          <div className="mb-2 font-bold text-cyan-300">Component Active</div>
          <div className="text-sm text-slate-400">
            Rendering and responding to state changes
          </div>
          {phase === "mounted" && (
        ...
        >
          <div className="mb-2 font-bold text-red-300">Cleanup Function</div>
          <div className="text-sm text-slate-400">
            Runs before re-running effect or unmounting
          </div>
          {phase === "unmounting" && (
            <div className="mt-2 animate-pulse text-xs text-red-400">
              → Cleaning up...
            </div>
          )}
        </div>
      ...
      <div className="flex gap-4">
        <button
          ...
          className="rounded-lg bg-sky-600 px-6 py-3 font-semibold text-white transition-colors hover:bg-sky-500 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-500"
        >
          Mount
        </button>
        <button
          ...
          className="rounded-lg bg-cyan-600 px-6 py-3 font-semibold text-white transition-colors hover:bg-cyan-500 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-500"
        >
          Update State
        </button>
        <button
          ...
          className="rounded-lg bg-red-600 px-6 py-3 font-semibold text-white transition-colors hover:bg-red-500 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-500"
        >
          Unmount
        </button>
      </div>
      ...
      <div className="mt-6 rounded border border-slate-700 bg-slate-950/50 p-4">
        <div className="mb-2 font-semibold text-cyan-400">Key Takeaway:</div>
        <p className="text-sm text-slate-300">
          Every component follows this lifecycle. Effects run during mount and
          updates. Cleanup runs before re-running effects and during unmount.
          Understanding this flow is essential for managing side effects
          properly—just like Christof needed to understand that Truman's
          lifecycle would eventually end, and he needed a cleanup function (the
          door) to handle it gracefully.
        </p>
      </div>
    </div>
  );
...
function PhaseIndicator({ label, active }: { label: string; active: boolean }) {
  return (
    <div className="flex flex-col items-center">
      <div
        className={`mb-2 h-4 w-4 rounded-full transition-all ${
          active
            ? "animate-pulse bg-sky-400 ring-4 ring-sky-400/30"
            : "bg-slate-600"
        }`}
      />
      <span
        className={`text-xs font-semibold ${active ? "text-sky-300" : "text-slate-500"}`}
      >
        {label}
      </span>
    </div>
