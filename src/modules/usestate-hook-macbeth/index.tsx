...
export default function UseStateHookMacbeth(): JSX.Element {
  ...
  const [kingState, setKingState] = useState<{ title: string; ambition: number }>({
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="rounded-xl border border-amber-500/30 bg-slate-900/80 p-4">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="mb-1 block text-sm text-slate-400">Ambition Level</label>
                    <input
                      type="range"
                      min="0"
                      max="100"
                    ...
                    />
                    <div className="mt-1 text-center text-xs text-amber-300">{kingState.ambition}</div>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="rounded bg-slate-800/30 p-3 text-center">
                      <div className="text-xs text-slate-500">Current Title</div>
                      <div className="truncate font-medium">{kingState.title}</div>
                    </div>
                  </div>
                  <button
                    onClick={resetDemos}
              ...
              <div className="rounded-xl border border-slate-700 bg-slate-900/50 p-5">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between border-b border-slate-800 pb-2">
                    <span className="text-sm text-slate-400">Macbeth (Thane)</span>
                    <span className="text-sm font-medium">Functional Component</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-800 pb-2">
                    <span className="text-sm text-slate-400">Witches' Prophecy</span>
                    <span className="text-sm font-medium">useState() call</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-800 pb-2">
                    <span className="text-sm text-slate-400">Ambition for the Throne</span>
                    <span className="text-sm font-medium">State Variable</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-800 pb-2">
                    <span className="text-sm text-slate-400">Lady Macbeth's Push</span>
                    <span className="text-sm font-medium">Setter Function</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-800 pb-2">
                    <span className="text-sm text-slate-400">Murder, Tyranny</span>
                    <span className="text-sm font-medium">Re-renders & Side Effects</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-slate-400">The Misty Heath</span>
                    <span className="text-sm font-medium">Component Context</span>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-amber-500/30 bg-amber-950/20 p-4">
                ...
                <p className="text-sm text-amber-200/80">
                  {chapter === 0 && "`useState` hooks state onto a component, transforming it from static to dynamic."}
                  {chapter === 1 && "Changing a regular variable doesn't trigger a re-render. You need the state hook."}
                  {chapter === 2 && "The setter function provides the only proper way to update state and cause a re-render."}
                  {chapter === 3 && "Managed state drives predictable UI updates. Unmanaged state causes bugs and staleness."}
                  {chapter === 4 && "A component's rendered output (UI) is a direct expression of its current state."}
                </p>
              </div>
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-900/30 p-4">
                ...
                <p className="mt-2 text-right text-xs text-slate-500">— Macbeth, Act I, Scene VII</p>
              </div>
            </div>
          }
        >
            ...
            {chapter === 0 && (
              <div className="space-y-6">
                <div className="grid gap-6 lg:grid-cols-2">
                  <div className="space-y-4">
                    <h4 className="font-bold text-amber-100">The Prophecy (Initial State)</h4>
                    <p className="text-sm text-slate-400">
                      The hook is called, initializing the state. The component now possesses internal data.
                    </p>
                    <div className="rounded-lg border border-amber-500/30 bg-amber-950/10 p-4">
                      <div className="font-mono text-sm">
                        ...
                        <span className="text-green-400">"Thane of Glamis"</span>
                        <span className="text-slate-300">)</span>
                  ...
                  <div className="flex flex-col items-center justify-center rounded-lg border border-slate-700 bg-slate-900/50 p-6">
                    ...
                    <div className="text-center">
                      <div className="text-xs uppercase text-slate-500">Current Title</div>
                      <div className="text-2xl font-bold text-amber-300">Thane of Glamis</div>
                    </div>
                  </div>
                </div>
                <CodeBlock
              ...
              <div className="space-y-6">
                <div className="rounded-lg border border-red-500/30 bg-red-950/10 p-6">
                  <div className="mb-4 flex items-center gap-2">
                    <div className="rounded-full bg-red-500/20 px-3 py-1 text-sm font-bold text-red-300">❌ Anti-Pattern Demo</div>
                  </div>
                  <p className="mb-4 text-sm text-slate-400">
                    Click "Hear Prophecy". The variable updates internally (check console), but the UI does <strong>not</strong> re-render to show the new value.
                  </p>
                  <div className="grid gap-6 md:grid-cols-2">
                    <div className="space-y-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-sm text-slate-500">Variable: {directVarRef.current}</div>
                      </div>
                      <div className="rounded border border-slate-700 bg-slate-900 p-4 text-center">
                        <div className="text-xs text-slate-500">Rendered Title (Stale)</div>
                        <div className="text-2xl font-bold text-red-400">Thane #{directVar}</div>
                        <div className="mt-2 text-xs text-red-500/80">UI not updating!</div>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <div className="rounded bg-slate-800/50 p-3">
                        <div className="text-xs text-slate-500">Internal Variable (Ref)</div>
                        <div className="font-mono text-xl tabular-nums text-red-300">{directVarRef.current}</div>
                      </div>
                      <div className="rounded bg-slate-800/50 p-3">
                        <div className="text-xs text-slate-500">UI Variable (State)</div>
                        <div className="font-mono text-xl tabular-nums text-amber-300">{directVar}</div>
                      </div>
                      <div className="text-xs text-slate-500">
                        The ref updates instantly. The state only updates when we force it, proving the disconnect.
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {chapter === 2 && (
              <div className="space-y-6">
                <div className="rounded-lg border border-amber-500/30 bg-amber-950/10 p-6">
                  <div className="mb-4 flex items-center gap-2">
                    <div className="rounded-full bg-amber-500/20 px-3 py-1 text-sm font-bold text-amber-300">✅ Setter Function Demo</div>
                  </div>
                  <p className="mb-4 text-sm text-slate-400">
                    Click "Seize Power". The <code>setCount</code> function updates the state, triggering an immediate re-render.
                  </p>
                  <div className="grid gap-6 md:grid-cols-2">
                    <div className="space-y-4">
                      <button
                      ...
                      <div className="rounded border border-amber-500/30 bg-amber-950/20 p-4 text-center">
                        <div className="text-xs uppercase text-amber-500/80">Ambition Level</div>
                        <div className="text-5xl font-bold tabular-nums text-amber-300">{count}</div>
                    ...
                    <div className="space-y-3">
                      <div className="rounded bg-slate-800/50 p-3">
                        <div className="text-xs text-slate-500">Component Re-renders</div>
                        <div className="font-mono text-2xl tabular-nums text-cyan-300">{renderCount}</div>
                      </div>
                      <div className="text-xs text-slate-500">
                        Each click calls <code>setCount</code>, updating state and causing a re-render. The UI always reflects the current state.
                      </div>
                    </div>
                  </div>
                </div>
  ...
  // Triggers re-render, UI updates
                ...
                <div className="grid gap-4 rounded-lg border border-slate-700 bg-slate-900/50 p-4 md:grid-cols-2">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="font-bold text-red-300">Without useState</div>
                    </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="font-bold text-amber-300">With useState</div>
                    </div>
                    <ul className="list-inside list-disc space-y-1 text-sm text-slate-400">
            ...
            {chapter === 4 && (
              <div className="space-y-6">
                <div className="rounded-lg border border-amber-500/30 bg-amber-950/10 p-6">
                  <div className="mb-4">
                    <h4 className="font-bold text-amber-100">The King's State Drives Everything</h4>
                    <p className="text-sm text-slate-400">
                      The UI below is rendered entirely from the single <code>kingState</code> object. Changing any part updates the whole component.
                    </p>
                  </div>
                  <div className="grid gap-6 lg:grid-cols-3">
                    <div className="space-y-4 rounded-lg border border-slate-700 bg-slate-900/50 p-4">
                      <div className="text-center">
                        ...
                        <div className={`text-xl font-bold ${kingState.ambition > 50 ? "text-amber-300" : "text-slate-300"}`}>
                          {kingState.ambition > 50 ? "King of Scotland" : kingState.title}
                        </div>
                      </div>
                    </div>
                    <div className="space-y-4 rounded-lg border border-slate-700 bg-slate-900/50 p-4">
                      <div>
                        <div className="text-xs uppercase text-slate-500">Ambition Level</div>
                        <div className="mt-1 flex items-center gap-2">
                          <div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-700">
                            <div
                              className="h-full rounded-full bg-gradient-to-r from-amber-600 to-amber-400"
                              style={{ width: `${Math.min(100, kingState.ambition)}%` }}
                            ></div>
                          </div>
                          <span className="font-mono tabular-nums">{kingState.ambition}</span>
                        </div>
                      </div>
                      <div>
                        <div className="text-xs uppercase text-slate-500">Royal Actions</div>
                        <div className="mt-2 space-y-1">
                          {kingState.ambition > 20 && <div className="text-sm">🔪 Met with Three Witches</div>}
                          {kingState.ambition > 40 && <div className="text-sm">⚔️ Ordered Duncan's Murder</div>}
                          {kingState.ambition > 60 && <div className="text-sm">🏰 Crowned at Scone</div>}
                          {kingState.ambition > 80 && <div className="text-sm">👥 Purged the Nobles</div>}
                          {kingState.ambition <= 20 && <div className="text-sm text-slate-600">- No decisive actions -</div>}
                        </div>
                      </div>
                    </div>
                    <div className="space-y-4">
                      <button
                        onClick={() =>
                          setKingState((prev) => ({
                            ...
                            actions: [...prev.actions, `Ambition surged to ${prev.ambition + 15}`],
                          }))
                        }
                      ...
                      <div className="rounded bg-slate-800/30 p-3 text-center">
                        <div className="text-xs text-slate-500">State-Driven Renders</div>
                        <div className="font-mono text-xl tabular-nums text-cyan-300">{renderCount}</div>
                      </div>
                    </div>
                  </div>
                </div>
