...
export default function StrictModePrecognition(): JSX.Element {
  ...
  const [isStrictModeEnabled, setIsStrictModeEnabled] =
...
createRoot(document.getElementById('root')).render(
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950/40 to-slate-950 font-sans text-slate-300">
      {/* Header */}
      <header className="top-0 z-10 border-b border-slate-800 bg-slate-950/80 backdrop-blur-sm">
        <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
          <div className="mb-2 flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <Brain className="h-7 w-7 text-blue-400 sm:h-8 sm:w-8" />
          ...
          <p className="text-base font-medium text-blue-400 sm:text-lg">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
          ...
          <div className="space-y-8 lg:col-span-8">
            ...
            <section className="rounded-xl border border-slate-800 bg-slate-900/50 p-6">
              ...
              {/* Memorable Quote */}
              <div className="border-l-4 border-blue-500 bg-blue-950/20 py-2 pl-4 italic">
                {chapter === 0 && "We don't chase bugs; we prevent them."}
                {chapter === 1 &&
                  "Every time I fix one leak, another one springs."}
                {chapter === 2 &&
                  "It's not just seeing the future. It's running it twice to be sure."}
                {chapter === 3 &&
                  "It's not about being faster; it's about being correct."}
                {chapter === 4 &&
            ...
            <section className="rounded-xl border border-slate-800 bg-slate-900/50 p-6">
              ...
              {chapter === 0 && (
                <div className="space-y-6">
                  <div className="rounded-lg border border-blue-500/30 bg-blue-950/30 p-4">
                    <div className="mb-2 flex items-center gap-2">
                      ...
                      <span className="font-medium">
                        PreCrime Vision Detected
                      </span>
                    </div>
                    <p className="text-sm">
                      Deprecated schema `v2.7` detected in data layer. Update to
                      `v3.0` to prevent data corruption.
                    </p>
                  </div>
                  <CodeBlock
                    code={strictModeCode}
                  ...
                  <p className="text-sm text-slate-400">
                    Strict Mode wraps your app to enable additional checks and
                  ...
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-3" ref={parent}>
                        {PROFILES.map((profile) => (
                          <button
                            ...
                            className={`rounded-lg border p-4 transition-all ${activeProfileId === profile.id ? "border-blue-500 bg-blue-950/30" : "border-slate-700 bg-slate-800/50 hover:border-slate-600"}`}
                          >
                      ...
                      <div className="flex flex-wrap gap-3 pt-4">
                        <button
                          onClick={() =>
                            setIsStrictModeEnabled(!isStrictModeEnabled)
                          }
                          className={`flex items-center gap-2 rounded px-4 py-2 ${isStrictModeEnabled ? "bg-green-600 hover:bg-green-700" : "bg-slate-700 hover:bg-slate-600"}`}
                        >
                          {isStrictModeEnabled ? (
                        ...
                        <button
                          onClick={resetDemo}
                          className="flex items-center gap-2 rounded bg-slate-700 px-4 py-2 hover:bg-slate-600"
                        >
                    ...
                    <div className="space-y-4">
                      ...
                      <div
                        className={`rounded-lg border p-4 ${chapter === 1 && staleFlashCount > 0 ? "animate-pulse border-red-500/50 bg-red-950/20" : "border-slate-700 bg-slate-800/50"}`}
                      >
                        <div className="mb-2 text-sm text-slate-400">
                          Current Data Stream
                        </div>
                        <div className="font-mono text-sm whitespace-pre">
                          {profileData || "No data loaded"}
                        </div>
                      </div>
                  ...
                  <div className="space-y-4 border-t border-slate-800 pt-4">
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      ...
                      <CodeBlock
                        ...
                        variant={
                          chapter === 3 || chapter === 4 ? "success" : "default"
                        }
                        title="// ✅ With Cleanup"
                        language="jsx"
              ...
              {chapter === 4 && (
                <div className="mt-6 rounded-lg border border-green-500/30 bg-green-950/20 p-4">
                  <div className="mb-2 flex items-center gap-2">
                    <CheckCircle className="h-5 w-5 text-green-500" />
                    <span className="font-medium">All Systems Stable</span>
                  </div>
                  <p className="text-sm">
                    With Strict Mode enabled and proper cleanup implemented, the
                    application runs predictably. Warnings during development
            ...
            <nav className="flex items-center justify-between border-t border-slate-800 pt-6">
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-lg bg-blue-600 px-6 py-3 text-white transition-colors hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-30"
              >
                Next →
              </button>
            </nav>
          ...
          <div className="space-y-6 lg:col-span-4">
              ...
              <div className="max-h-[400px] space-y-3 overflow-y-auto">
                {warnings.length === 0 ? (
                  <p className="text-sm text-slate-400 italic">
                    No warnings detected. System stable.
                  </p>
                ) : (
                ...
                <div className="text-sm text-slate-400">
                  {leakedTimers > 0 && (
                    <div className="flex items-center gap-2 text-red-400">
                      <AlertTriangle className="h-4 w-4" />
                      {leakedTimers} potential memory leak
                      {leakedTimers !== 1 ? "s" : ""} detected
                    </div>
                  )}
                  {leakedTimers >= 40 && (
            ...
            {/* Key Concepts */}
            <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-6">
              <h3 className="mb-4 text-xl font-bold">Strict Mode Guardrails</h3>
              <ul className="space-y-3">
                <li className="flex items-start gap-2">
                  <div className="mt-1.5 h-2 w-2 rounded-full bg-blue-500"></div>
                  <span className="text-sm">
                    Double-invokes render/effects to expose impurities
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <div className="mt-1.5 h-2 w-2 rounded-full bg-blue-500"></div>
