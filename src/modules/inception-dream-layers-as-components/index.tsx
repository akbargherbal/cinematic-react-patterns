...
import { Brain, Coffee, Building2, Hotel, Hospital, Layers, ChevronLeft, ChevronRight, RotateCcw, AlertTriangle, CheckCircle } from "lucide-react";
...
export default function InceptionDreamLayers(): JSX.Element {
  ...
  const [componentStack, setComponentStack] = useState<DreamLayer[]>([
    { id: "city", name: "Rainy City", description: "Yusuf's level", color: "bg-blue-500/20", state: { rain: true, traffic: "heavy" } },
    { id: "hotel", name: "Luxury Hotel", description: "Arthur's level", color: "bg-amber-500/20", state: { gravity: "normal", ambiance: "calm" } },
    { id: "hospital", name: "Mountain Hospital", description: "Eames's level", color: "bg-cyan-500/20", state: { snow: true, temperature: "cold" } },
  ]);
  const [activeKicks, setActiveKicks] = useState<string[]>([]);
  const [comparisonView, setComparisonView] = useState<"monolith" | "components">("monolith");

  const chapters: Chapter[] = [
    {
      title: "The Parisian Cafe",
      content: "In the cold warehouse, Cobb teaches Ariadne the first rule of dream architecture: every world must be self-contained. She designs a Parisian cafe with meticulous detail—the wobble of the third table, the aroma of coffee. When rendered through the PASIV device, the cafe exists in perfect isolation. 'This world only knows what you told it. Nothing more.' This establishes the fundamental principle of React components: isolated, reusable units with defined boundaries.",
...
<div className="fixed inset-0 bg-[url('data:image/svg+xml,%3Csvg%20width=%22100%22%20height=%22100%22%20viewBox=%220%200%20100%20100%22%20xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cpath%20d=%22M0,0%20L100,0%20L100,100%20L0,100%20Z%22%20fill=%22none%22%20stroke=%22%2523334d5c%22%20stroke-width=%220.5%22/%3E%3C/svg%3E')] opacity-10 pointer-events-none"></div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
          ...
          <div className="flex items-center justify-between gap-4 sm:gap-6 mb-2 flex-wrap">
            <div className="flex items-center gap-3">
              <Brain className="text-cyan-500 w-7 h-7 sm:w-8 sm:h-8" />
              <h1 className="text-2xl sm:text-3xl font-bold">Inception</h1>
            </div>
          ...
          <p className="text-base sm:text-lg text-cyan-500 font-medium">
          ...
          <div className="lg:col-span-7">
            ...
            <div className="prose prose-invert prose-lg max-w-none mb-8 sm:mb-12">
              ...
              <div className="mt-4 text-sm text-slate-500 flex items-center gap-2">
                <span className="px-2 py-1 bg-slate-800 rounded">Atmosphere:</span>
                <span>{currentChapter.atmosphere}</span>
              </div>
            </div>

            {/* Interactive demo section */}
            <section className="bg-slate-900/50 border border-slate-700 rounded-xl p-4 sm:p-6 mb-8 sm:mb-12">
              <h3 className="text-lg sm:text-xl font-semibold mb-4 text-amber-500">
              ...
              <div ref={animationParent}>
                ...
                {chapter === 0 && (
                  <div className="space-y-6">
                    <div className="p-4 sm:p-6 bg-amber-950/20 border border-amber-500/30 rounded-lg">
                      <div className="flex items-center gap-3 mb-4">
                        <Coffee className="w-5 h-5 text-amber-500" />
                        <h4 className="text-lg font-semibold">Parisian Cafe Component</h4>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <p className="text-sm text-slate-400 mb-2">Ambiance:</p>
                          <div className="flex items-center gap-4">
                            <button
                              onClick={() => setCafeAmbiance("calm")}
                              className={`px-4 py-2 rounded transition-all ${cafeAmbiance === "calm" ? "bg-amber-600 text-white" : "bg-slate-800 text-slate-300"}`}
                            >
                              Calm
                            </button>
                            <button
                              onClick={() => setCafeAmbiance("busy")}
                              className={`px-4 py-2 rounded transition-all ${cafeAmbiance === "busy" ? "bg-amber-600 text-white" : "bg-slate-800 text-slate-300"}`}
                            >
                              Busy
                            </button>
                          </div>
                          <div className="mt-4 space-y-2">
                            <div className="flex justify-between text-sm">
                              ...
                              <span className="font-mono">{cafeAmbiance === "busy" ? "12-20" : "3-5"}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                              <span>Sound Level:</span>
                              <span className="font-mono">{cafeAmbiance === "busy" ? "Murmuring" : "Quiet"}</span>
                            </div>
                          </div>
                        </div>
                        <div className="border-l border-slate-700 pl-4 sm:pl-6">
                          <p className="text-sm text-slate-400 mb-2">Component State:</p>
                          <div className="space-y-2">
                            <div className="flex items-center gap-2">
                              <div className={`w-3 h-3 rounded-full ${cafeAmbiance === "calm" ? "bg-green-500" : "bg-amber-500"}`}></div>
                              <span className="text-sm">Isolated from other components</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <div className="w-3 h-3 rounded-full bg-blue-500"></div>
                              <span className="text-sm">Cleanup on unmount</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <div className="w-3 h-3 rounded-full bg-cyan-500"></div>
                              <span className="text-sm">Props define interface</span>
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                {chapter === 1 && (
                  <div className="space-y-6">
                    <div className="p-4 sm:p-6 bg-red-950/20 border border-red-500/30 rounded-lg">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <Building2 className="w-5 h-5 text-red-500" />
                          <h4 className="text-lg font-semibold">Monolithic Dream</h4>
                        </div>
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => setMonolithMode("chaos")}
                            className={`px-3 py-1 text-sm rounded ${monolithMode === "chaos" ? "bg-red-600 text-white" : "bg-slate-800 text-slate-300"}`}
                          >
                            ❌ Show Bug
                          </button>
                          <button
                            onClick={() => setMonolithMode("order")}
                            className={`px-3 py-1 text-sm rounded ${monolithMode === "order" ? "bg-green-600 text-white" : "bg-slate-800 text-slate-300"}`}
                          >
                            ✅ Show Fix
                          </button>
                        </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                        <div className="p-3 bg-blue-950/30 rounded">
                          <p className="text-sm font-medium mb-1">Rainy City</p>
                          <p className="text-xs text-slate-400">State: {monolithMode === "chaos" ? "Rain affects everything" : "Isolated"}</p>
                        </div>
                        <div className="p-3 bg-amber-950/30 rounded">
                          <p className="text-sm font-medium mb-1">Luxury Hotel</p>
                          <p className="text-xs text-slate-400">Gravity: {monolithMode === "chaos" ? "Unstable" : "Normal"}</p>
                        </div>
                        <div className="p-3 bg-cyan-950/30 rounded">
                          <p className="text-sm font-medium mb-1">Mountain Hospital</p>
                          <p className="text-xs text-slate-400">Snow: {monolithMode === "chaos" ? "Caused by rain" : "Independent"}</p>
                        </div>
                      </div>
                      ...
                      {monolithMode === "chaos" && (
                        <div className="space-y-3">
                          <div className="flex items-center justify-between text-sm">
                            ...
                            <span className="font-mono text-red-500">{leakCount}</span>
                          </div>
                          <div className="flex items-center justify-between text-sm">
                          ...
                          <div className="text-xs text-red-400">
                            ⚠️ Without component isolation, changing one state affects everything
                          </div>
                        </div>
                      )}
                      ...
                      <button
                        ...
                        className="mt-4 px-4 py-2 bg-slate-800 text-slate-300 rounded text-sm hover:bg-slate-700 transition-colors"
                      >
                    ...
                    <CodeBlock
                      ...
                      title={monolithMode === "chaos" ? "// ❌ Monolithic Anti-Pattern" : "// ⚠️ Problem Identified"}
                      defaultExpanded={true}
                    />
                  </div>
                )}
                ...
                {chapter === 2 && (
                  <div className="space-y-6">
                    <div className="p-4 sm:p-6 bg-slate-900 border border-slate-700 rounded-lg">
                      <div className="flex items-center gap-3 mb-4">
                        <Layers className="w-5 h-5 text-cyan-500" />
                        <h4 className="text-lg font-semibold">Component Stack</h4>
                      </div>
                      ...
                      <div className="space-y-4">
                        {componentStack.map((layer, index) => (
                          <div
                            key={layer.id}
                            className={`p-4 rounded-lg border ${layer.color} border-opacity-30`}
                          >
                            ...
                            <p className="text-sm text-slate-400 mb-2">{layer.description}</p>
                            <div className="text-xs space-y-1">
                              {Object.entries(layer.state).map(([key, value]) => (
                      ...
                      <div className="mt-4 p-3 bg-slate-800/50 rounded">
                        <p className="text-sm text-slate-300">
                          <span className="text-cyan-400 font-medium">Key Insight:</span> Each component receives only the props it needs. The hotel doesn't know about the city's rain, and the hospital doesn't know about the hotel's gravity.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="p-4 sm:p-6 bg-slate-900 border border-slate-700 rounded-lg">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <h4 className="text-lg font-semibold">Architecture Comparison</h4>
                        </div>
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => setComparisonView("monolith")}
                            className={`px-3 py-1 text-sm rounded ${comparisonView === "monolith" ? "bg-red-600 text-white" : "bg-slate-800 text-slate-300"}`}
                          >
                            Monolith
                          </button>
                          <button
                            onClick={() => setComparisonView("components")}
                            className={`px-3 py-1 text-sm rounded ${comparisonView === "components" ? "bg-green-600 text-white" : "bg-slate-800 text-slate-300"}`}
                          >
                            Components
                          </button>
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className={`p-4 rounded-lg ${comparisonView === "monolith" ? "bg-red-950/20 border-red-500/30" : "bg-slate-800/30 border-slate-700"} border`}>
                        ...
                        <div className={`p-4 rounded-lg ${comparisonView === "components" ? "bg-green-950/20 border-green-500/30" : "bg-slate-800/30 border-slate-700"} border`}>
                          <div className="flex items-center gap-2 mb-3">
                      ...
                      <div className="mt-4 p-3 bg-slate-800/50 rounded">
                        <p className="text-sm text-slate-300">
                          <span className="text-amber-400 font-medium">Cobb's Wisdom:</span> "The hotel only accepts what we choose to pass down. It can't poison the ones next to it."
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    <div className="p-4 sm:p-6 bg-slate-900 border border-slate-700 rounded-lg">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <h4 className="text-lg font-semibold">Synchronized Kick Cascade</h4>
                      </div>
                      
                      <div className="space-y-4">
                        {[
                          { id: "city", name: "Level 1: Rainy City", description: "Van hits the water" },
                          { id: "hotel", name: "Level 2: Luxury Hotel", description: "Elevator explosion" },
                          { id: "hospital", name: "Level 3: Mountain Hospital", description: "Defibrillator shock" }
                        ].map((level) => (
                          <div
                            key={level.id}
                            className={`p-4 rounded-lg border ${activeKicks.includes(level.id) ? "border-cyan-500 bg-cyan-950/20" : "border-slate-700 bg-slate-800/30"} transition-all duration-300`}
                          >
                            <div className="flex items-center justify-between mb-2">
                              <h5 className="font-medium">{level.name}</h5>
                              {activeKicks.includes(level.id) ? (
                                <span className="px-2 py-1 bg-cyan-600 text-white text-xs rounded animate-pulse">
                                  KICK ACTIVE
                                </span>
                              ) : (
                      ...
                      <div className="mt-4 p-3 bg-slate-800/50 rounded">
                        <p className="text-sm text-slate-300">
                          <span className="text-cyan-400 font-medium">The Cascade:</span> Each component waits for its specific callback prop. Level 1 triggers Level 2, which triggers Level 3. Clean data flow through props enables complex coordination.
                        </p>
                      </div>
                    </div>
            ...
            {/* Navigation */}
            <nav className="flex items-center justify-between pt-4 border-t border-slate-800">
              <button
                onClick={() => setChapter(Math.max(0, chapter - 1))}
                disabled={chapter === 0}
                className="flex items-center gap-2 px-4 sm:px-6 py-3 bg-slate-800 text-slate-300 rounded-lg hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
              >
              ...
              <div className="flex flex-col items-center">
                <div className="flex items-center gap-2 mb-1">
                  {chapters.map((_, idx) => (
                    <div
                      key={idx}
                      className={`w-2 h-2 rounded-full ${idx === chapter ? "bg-cyan-500" : "bg-slate-700"}`}
                    />
                  ))}
                </div>
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 sm:px-6 py-3 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
              >
          ...
          <div className="lg:col-span-5 mt-8 lg:mt-0 lg:pl-8">
            <div className="sticky top-8 space-y-6">
              ...
              <div className="bg-slate-900/50 border border-slate-700 rounded-xl p-4 sm:p-6">
                <h3 className="text-lg font-semibold mb-4 text-amber-500">Metaphor Registry</h3>
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-medium">Dream Layer = React Component</p>
                      <p className="text-sm text-slate-400">Self-contained, reusable UI unit with own logic</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-400">Designs and builds component structure</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-400">Data passed from parent to child component</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-400">Allows child to communicate back to parent</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Learning checkpoint */}
              <div className="bg-slate-900/50 border border-slate-700 rounded-xl p-4 sm:p-6">
                <h3 className="text-lg font-semibold mb-4 text-green-500">Learning Checkpoint</h3>
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <CheckCircle className="w-5 h-5 text-green-500" />
                    <span className="font-medium">Chapter {chapter + 1} Complete</span>
                  </div>
                  <p className="text-sm text-slate-300">
                    {chapter === 0 && "✓ You understand that components are isolated units with their own state"}
                    {chapter === 1 && "✓ You recognize the chaos of monolithic components without boundaries"}
                    {chapter === 2 && "✓ You can break complex UIs into composable components with props"}
                    {chapter === 3 && "✓ You can compare component isolation vs. monolithic entanglement"}
                    {chapter === 4 && "✓ You understand how callback props enable component communication"}
                  </p>
                </div>
              </div>
              ...
              {/* Key insights */}
              <div className="bg-slate-900/50 border border-slate-700 rounded-xl p-4 sm:p-6">
                <h3 className="text-lg font-semibold mb-4 text-amber-500">Key Insights</h3>
                <ul className="space-y-3">
                  <li className="flex items-start gap-2">
                    <div className="w-2 h-2 mt-2 rounded-full bg-cyan-500"></div>
                    <p className="text-sm">Components create <span className="text-cyan-400">isolated worlds</span> with clear boundaries</p>
                  </li>
                  <li className="flex items-start gap-2">
                    <div className="w-2 h-2 mt-2 rounded-full bg-green-500"></div>
                    <p className="text-sm">Props define the <span className="text-green-400">interface</span> between components</p>
                  </li>
                  <li className="flex items-start gap-2">
                    <div className="w-2 h-2 mt-2 rounded-full bg-amber-500"></div>
                    <p className="text-sm">Monolithic components cause <span className="text-amber-400">state leakage</span> and unpredictable bugs</p>
                  </li>
                  <li className="flex items-start gap-2">
                    <div className="w-2 h-2 mt-2 rounded-full bg-blue-500"></div>
                    <p className="text-sm">Callback props enable <span className="text-blue-400">clean communication</span> between components</p>
                  </li>
                </ul>
              </div>
            </div>
