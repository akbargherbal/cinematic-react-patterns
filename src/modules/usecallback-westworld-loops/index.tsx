    ...
    return (
      <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700">
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className="font-mono text-sm">Teddy Flood (Memoized Child)</span>
        </div>
        <button
          onClick={onDropCan}
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-4 md:py-6">
          <div className="flex items-center justify-between gap-4 md:gap-6 mb-2 flex-wrap">
            <div className="flex items-center gap-3">
              <Repeat className="text-amber-500 w-6 h-6 md:w-8 md:h-8" />
              <h1 className="text-xl md:text-2xl lg:text-3xl font-bold">Westworld</h1>
            </div>
            <p className="text-xs md:text-sm text-slate-400">Series • Dolores &amp; Teddy • 2016</p>
          </div>
          <p className="text-base md:text-lg text-amber-500 font-medium">useCallback Hook</p>
        </div>
      </header>

      <main className="max-w-7xl mx-auto mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-7">
          <div className="prose prose-invert prose-lg max-w-none mb-8 p-4 md:p-6 bg-slate-900/50 rounded-xl border border-slate-800">
            <h2 className="text-2xl md:text-3xl font-bold mb-4 text-amber-100">{currentChapter.title}</h2>
            <div className="leading-relaxed whitespace-pre-line">{currentChapter.content}</div>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 mt-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm font-mono tabular-nums opacity-70">
        ...
        <div className="lg:col-span-5">
          <div className="bg-slate-900/80 border border-amber-500/30 rounded-xl p-4 md:p-6 backdrop-blur-sm sticky top-24">
            <h3 className="text-lg md:text-xl font-bold mb-4 flex items-center gap-2 text-amber-300">
            ...
            {chapter === 0 && (
              <div className="space-y-6">
                <p className="text-sm opacity-80">The baseline: Dolores passes a callback to Teddy. Click to simulate the "Drop the Can" action.</p>
                <div className="space-y-4">
                  <div className="bg-slate-800/50 p-4 rounded-lg">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-3 h-3 rounded-full bg-blue-500"></div>
                      <span className="font-mono text-sm">Dolores Abernathy (Parent)</span>
                    </div>
                    <button
                      onClick={() => setCanCount(c => c + 1)}
                      className="px-4 py-2 bg-blue-700 hover:bg-blue-600 text-white rounded-lg transition-colors w-full"
                    >
                      🥛 Drop Can of Milk
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-center">
                  <div className="bg-slate-800/30 p-3 rounded-lg">
                    <div className="text-2xl font-bold font-mono">{canCount}</div>
                    <div className="text-xs opacity-70">Cans Picked Up</div>
                  </div>
                  <div className="bg-slate-800/30 p-3 rounded-lg">
                    <div className="text-2xl font-bold font-mono">{teddyRenderCount}</div>
            ...
            {chapter === 1 && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <p className="text-sm opacity-80">Anti-pattern: New function on every Dolores render.</p>
                  <button
                    onClick={resetDemo}
                    className="px-3 py-1 text-xs bg-slate-800 hover:bg-slate-700 rounded flex items-center gap-1"
                  >
                ...
                <div className="space-y-4">
                  <div className="bg-slate-800/50 p-4 rounded-lg">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <span className="font-mono text-sm">Dolores (Re-renders: {doloresRenderCount})</span>
                    </div>
                ...
                <div className="space-y-4">
                  <div className="bg-red-950/30 border border-red-500/30 p-4 rounded-lg">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-bold text-red-400">Performance Spike</span>
                    </div>
                    ...
                    <div className="text-xs text-red-300 mt-2">System load due to cascading updates: {performanceSpike}%</div>
                  </div>
                  <CodeBlock
                    code={brokenCode}
                    variant="error"
            ...
            {chapter === 2 && (
              <div className="space-y-6">
                <p className="text-sm opacity-80">Solution: useCallback stabilizes the function identity.</p>
                <div className="space-y-4">
                  <div className="bg-slate-800/50 p-4 rounded-lg">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-3 h-3 rounded-full bg-blue-500"></div>
                      <span className="font-mono text-sm">Dolores (Optimized with useCallback)</span>
                    </div>
                    <button
                      onClick={triggerRerender}
                      className="px-3 py-2 bg-blue-700 hover:bg-blue-600 text-white rounded-lg text-sm"
                ...
                <div className="bg-emerald-950/30 border border-emerald-500/30 p-4 rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-bold text-emerald-400">Optimal Performance</span>
                  </div>
                  <div className="text-sm">Teddy re-renders only when necessary. System load is minimal.</div>
                </div>
                <CodeBlock
                  code={fixedCode}
                  variant="success"
            ...
            {chapter === 3 && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className={`px-3 py-1 rounded text-sm font-medium transition-colors ${mode === "broken" ? "bg-red-900/50 text-red-300 border border-red-700" : "bg-emerald-900/50 text-emerald-300 border border-emerald-700"}`}
                    >
                      {mode === "broken" ? "❌ Show Broken" : "✅ Show Fixed"}
                    </button>
                    <span className="text-xs opacity-70">{mode === "broken" ? "Wasteful Loop" : "Optimized Loop"}</span>
                  </div>
                  <button
                    onClick={resetDemo}
                    className="px-3 py-1 text-xs bg-slate-800 hover:bg-slate-700 rounded flex items-center gap-1"
                ...
                <div className="space-y-4">
                  <div className="bg-slate-800/50 p-4 rounded-lg">
                    <div className="mb-4">
                      <label className="block text-sm mb-2 opacity-80">Reverie (Dependency):</label>
                      <input
                        type="text"
                        value={reverie}
                      ...
                      />
                      <div className="text-xs opacity-60 mt-1">Changing this recreates the callback when dependency array includes it.</div>
                    </div>
                    <button
                      onClick={triggerRerender}
                      className="px-3 py-2 bg-blue-700 hover:bg-blue-600 text-white rounded-lg text-sm"
                  ...
                  <TeddyChild 
                    onDropCan={mode === "broken" ? handleDropCanBroken : handleDropCanWithDependency} 
                    onRender={() => setTeddyRenderCount(c => c + 1)}
                  />
                </div>
                <div className="grid grid-cols-3 gap-3 text-center text-sm">
                  <div className="bg-slate-800/30 p-3 rounded-lg">
                    <div className="font-bold font-mono">{doloresRenderCount}</div>
                    <div className="text-xs opacity-70">Dolores Renders</div>
                  </div>
                  <div className="bg-slate-800/30 p-3 rounded-lg">
                    <div className="font-bold font-mono">{teddyRenderCount}</div>
                    <div className="text-xs opacity-70">Teddy Renders</div>
                  </div>
                  <div className="bg-slate-800/30 p-3 rounded-lg">
                    <div className="font-bold font-mono">{canCount}</div>
                    <div className="text-xs opacity-70">Cans Picked Up</div>
                  </div>
                </div>
                <CodeBlock
                  ...
                  title={mode === "broken" ? "// ❌ Wasteful Loop" : "// ✅ Optimized with Dependency"}
                  defaultExpanded={true}
                />
                <div className="text-xs opacity-60">
                  <strong>Note:</strong> In "Fixed" mode, the callback only changes when the reverie dependency changes. Teddy re-renders only then.
                </div>
              </div>
            )}
            ...
            {chapter === 4 && (
              <div className="space-y-6">
                <p className="text-sm opacity-80">Summary: Stable foundations enable meaningful change.</p>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-800/50 p-4 rounded-lg border border-red-500/30">
                    <div className="text-center mb-2">
                      <AlertCircle className="w-6 h-6 text-red-500 mx-auto" />
                      <div className="font-bold text-red-400 mt-1">Without useCallback</div>
                    </div>
                  ...
                  <div className="bg-slate-800/50 p-4 rounded-lg border border-emerald-500/30">
                    <div className="text-center mb-2">
                      ...
                      <div className="font-bold text-emerald-400 mt-1">With useCallback</div>
                    </div>
                    <ul className="text-xs space-y-1 opacity-80">
                ...
                <div className="bg-slate-800/30 p-4 rounded-lg">
                  <h4 className="font-bold mb-2 text-amber-300">Key Takeaway</h4>
                  <p className="text-sm">useCallback memoizes functions to prevent unnecessary re-renders in optimized children. Provide a dependency array to control when the function should be recreated—only when its dependencies change.</p>
                </div>
                <button
                  onClick={resetDemo}
                  className="w-full px-4 py-3 bg-amber-800 hover:bg-amber-700 text-white rounded-lg transition-colors flex items-center justify-center gap-2"
                >
                  <RefreshCw className="w-4 h-4" /> Reset All Demos
                </button>
              </div>
            )}
          </div>
