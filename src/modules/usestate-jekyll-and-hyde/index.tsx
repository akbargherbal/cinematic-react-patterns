...
export default function UseStateJekyllAndHyde(): JSX.Element {
  ...
  const [showCorrectApproach, setShowCorrectApproach] =
...
// It's atomic and predictable`,
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-4 font-serif text-slate-300 md:p-8">
      {/* Header */}
      <header className="mb-8 border-b border-slate-800 bg-slate-950/80 backdrop-blur-sm md:mb-12">
        <div className="mx-auto max-w-7xl px-4 py-4 md:px-6 md:py-6">
          <div className="mb-2 flex flex-wrap items-center justify-between gap-4 md:gap-6">
            <div className="flex items-center gap-2 md:gap-3">
              <FlaskRound className="h-6 w-6 text-emerald-500 md:h-8 md:w-8" />
          ...
          <p className="text-sm font-medium text-emerald-500 md:text-base lg:text-lg">
      ...
      <main className="mx-auto max-w-7xl">
        <div className="grid grid-cols-1 gap-6 md:gap-8 lg:grid-cols-12">
          ...
          <div className="lg:col-span-8">
            ...
            {/* Navigation */}
            <nav className="mt-8 flex items-center justify-between border-t border-slate-800 pt-6 md:mt-12">
              <button
                onClick={() => setChapter(Math.max(0, chapter - 1))}
                disabled={isFirstChapter}
                className="flex items-center gap-2 rounded-lg bg-slate-800 px-4 py-2 text-slate-300 transition-colors hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-30 md:px-6 md:py-3"
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-white transition-colors hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-30 md:px-6 md:py-3"
          ...
          <div className="lg:col-span-4">
            <div className="sticky top-8">
              <div className="rounded-xl border border-slate-700 bg-slate-900/70 p-5 backdrop-blur-sm md:p-6">
                <div className="mb-4 flex items-center gap-2 md:mb-6">
                  <FlaskRound className="h-5 w-5 text-emerald-500" />
                ...
                {chapter === 0 && (
                  <div className="space-y-6">
                    <div className="text-center">
                      <div
                        className={`mb-4 text-5xl transition-all duration-500 md:text-6xl ${persona === "Hyde" ? "scale-90 rotate-2" : "scale-100"}`}
                      >
                        {persona === "Jekyll" ? "🧪" : "😈"}
                      </div>
                      <p className="mb-2 text-lg font-medium">
                        Current Persona:{" "}
                        <span
                          className={`font-bold ${persona === "Jekyll" ? "text-blue-400" : "text-red-400"}`}
                        >
                          {persona}
                        </span>
                      </p>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        ...
                        className="rounded-lg border border-blue-500/30 bg-blue-600/30 px-4 py-3 text-blue-300 transition-colors hover:bg-blue-600/50 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        Antidote → Jekyll
                      </button>
                      <button
                        ...
                        className="rounded-lg border border-red-500/30 bg-red-600/30 px-4 py-3 text-red-300 transition-colors hover:bg-red-600/50 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        Potion → Hyde
                      </button>
                    </div>
                ...
                {chapter === 1 && (
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div
                        className={`flex items-center justify-between ${isUiInSync ? "text-emerald-400" : "text-red-400"}`}
                      >
                    ...
                    <div className="space-y-3">
                      <button
                        ...
                        className="w-full rounded-lg border border-red-500/30 bg-red-600/30 px-4 py-3 text-red-300 transition-colors hover:bg-red-600/50 disabled:opacity-50"
                      >
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-sm">
                          ...
                          <span className="font-bold">
                            {manualMutationCount}/50
                          </span>
                        </span>
                        <button
                          onClick={resetMutationDemo}
                          className="flex items-center gap-1 rounded bg-slate-800 px-3 py-1.5 text-sm text-slate-300 transition-colors hover:bg-slate-700"
                        >
                  ...
                  <div className="space-y-6">
                    <div className="mb-2 flex items-center justify-center gap-4">
                      <button
                        ...
                        className={`rounded-lg px-4 py-2 transition-colors ${!showCorrectApproach ? "border border-red-500/30 bg-red-600/30 text-red-300" : "bg-slate-800 text-slate-400"}`}
                      >
                        ❌ Broken
                      </button>
                      <button
                        onClick={() => setShowCorrectApproach(true)}
                        className={`rounded-lg px-4 py-2 transition-colors ${showCorrectApproach ? "border border-emerald-500/30 bg-emerald-600/30 text-emerald-300" : "bg-slate-800 text-slate-400"}`}
                      >
                        ✅ Fixed
                      </button>
                    </div>
                    ...
                    {!showCorrectApproach ? (
                      <>
                        <div className="rounded-lg border border-red-500/30 bg-red-950/20 p-4">
                          <p className="text-sm text-red-300">
                            Direct mutation corrupts state. The UI shows "
                            {persona}" but manual overrides create hidden
                            inconsistencies.
                          </p>
                        </div>
                        <CodeBlock
                      ...
                      <>
                        <div className="rounded-lg border border-emerald-500/30 bg-emerald-950/20 p-4">
                          <p className="text-sm text-emerald-300">
                            The setter function declares a new state. The
                            component re-renders completely. UI stays perfectly
                            in sync.
                          </p>
                        </div>
                        <CodeBlock
                    ...
                    <button
                      onClick={() =>
                        setPersona((prev) =>
                          prev === "Jekyll" ? "Hyde" : "Jekyll",
                        )
                      }
                      className="w-full rounded-lg border border-emerald-500/30 bg-emerald-600/30 px-4 py-3 text-emerald-300 transition-colors hover:bg-emerald-600/50"
                    >
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="rounded-lg border border-red-500/30 bg-red-950/20 p-4">
                        ...
                        <button
                          ...
                          className="w-full rounded bg-red-600/30 px-3 py-2 text-sm text-red-300 transition-colors hover:bg-red-600/50"
                        >
                      ...
                      <div className="rounded-lg border border-emerald-500/30 bg-emerald-950/20 p-4">
                        ...
                        <button
                          ...
                          className="w-full rounded bg-emerald-600/30 px-3 py-2 text-sm text-emerald-300 transition-colors hover:bg-emerald-600/50"
                        >
                          Use setPersona()
                        </button>
                        <div className="mt-3 text-center">
                          <div className="font-mono text-2xl text-emerald-400">
                            {declarativeSuccesses}
                          </div>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <span
                          className={`font-mono ${persona === "Jekyll" ? "text-blue-400" : "text-red-400"}`}
                        >
                          {persona}
                        </span>
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-mono">
                          {declarativeSuccesses + mutationFailures}
                        </span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm">Success Rate:</span>
                        <span
                          className={`font-mono ${declarativeSuccesses > 0 ? "text-emerald-400" : "text-slate-400"}`}
                        >
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      {principles.map((principle, idx) => (
                        <button
                          ...
                          className={`w-full rounded-lg p-3 text-left transition-colors ${activePrinciple === idx ? "border border-emerald-500/30 bg-emerald-950/30" : "bg-slate-800/30 hover:bg-slate-800/50"}`}
                        >
                          <div className="mb-1 flex items-center gap-2">
                            <div
                              className={`flex h-5 w-5 items-center justify-center rounded-full text-xs ${activePrinciple === idx ? "bg-emerald-500 text-white" : "bg-slate-700 text-slate-400"}`}
                            >
                              {idx + 1}
                            </div>
                          ...
                          <p className="mt-1 text-xs text-slate-400">
                            {idx === 0 &&
                              "Components begin with a default value"}
                            {idx === 1 &&
                              "The setter is the only valid update mechanism"}
                            {idx === 2 &&
                              "Every state change triggers a full re-render"}
                          </p>
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="border-t border-slate-800 pt-4">
                      ...
                      <p className="text-sm text-emerald-300 italic">
                        "The potion is not a suggestion; it is a command to
                        re-render the soul."
                      </p>
                    </div>
                  </div>
