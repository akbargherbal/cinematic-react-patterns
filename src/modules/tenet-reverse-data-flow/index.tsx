...
const SyntaxHighlighter = ({ code, variant = "default" }: { code: string; variant?: "error" | "success" | "default" }) => {
  const colorScheme = {
    error: {
      bg: "bg-red-900/90",
      comment: "text-red-400",
      keyword: "text-red-300",
      function: "text-red-200",
      string: "text-red-100",
    ...
    success: {
      ...
      keyword: "text-green-300",
      function: "text-green-200",
      string: "text-green-100",
      operator: "text-green-300",
    },
    default: {
      bg: "bg-gray-900",
  ...
  const highlightSyntax = (text: string) => {
    ...
    return lines.map((line, lineIndex) => {
      // Keywords
      const keywordRegex = /\b(const|let|var|function|return|import|export|default|from|if|else|useEffect|useState|useContext|createContext)\b/g;
      ...
      // Split by segments and apply colors
      let lastIndex = 0;
      ...
      while ((match = functionRegex.exec(line)) !== null) {
        matches.push({ index: match.index, length: match[0].length, type: "function" });
      }
      ...
      matches.forEach((m, idx) => {
        ...
        if (m.index > currentIndex) {
          result.push(
            <span key={`${lineIndex}-${idx}-before`} className="text-gray-200">
              {line.substring(currentIndex, m.index)}
            </span>
          );
        }

        // Add matched text with color
        const colorClass = m.type === "keyword" ? colors.keyword
          : m.type === "string" ? colors.string
          : m.type === "function" ? colors.function
          : "text-gray-200";

        result.push(
          <span key={`${lineIndex}-${idx}-match`} className={colorClass}>
            {line.substring(m.index, m.index + m.length)}
          </span>
        );
      ...
      if (currentIndex < line.length) {
        result.push(
          <span key={`${lineIndex}-end`} className="text-gray-200">
            {line.substring(currentIndex)}
          </span>
        );
      }

      return <div key={lineIndex}>{result.length > 0 ? result : <span className="text-gray-200">{line}</span>}</div>;
    });
  };

  return (
    <pre className={`${colors.bg} p-4 rounded-lg overflow-x-auto text-xs leading-relaxed font-mono`}>
...
function Karen({ secret }) {
  ...
  return (
    <div className="animate-in fade-in duration-500 space-y-6">
      ...
      {/* Code Example - Collapsible with Syntax Highlighting */}
      <div className="bg-gray-900 rounded-2xl shadow-xl overflow-hidden">
        <div className="flex items-center justify-between p-4 bg-red-950/50">
          <h4 className="text-red-400 font-bold flex items-center gap-2">
            <Code2 size={20} />
            <span className="font-mono text-sm">// ❌ Prop Drilling Boilerplate</span>
          </h4>
          <button
            onClick={() => setShowCode(!showCode)}
            className="text-red-300 hover:text-red-100 flex items-center gap-2 text-xs transition-colors"
          >
        ...
        {showCode && (
          <div className="border-t border-red-900/50">
            <SyntaxHighlighter code={codeExample} variant="error" />
          </div>
        )}
      </div>
...
function Gretchen() {
  ...
  return (
    <div className="animate-in fade-in duration-500 space-y-6">
      ...
      <div className="bg-gray-900 rounded-2xl shadow-xl overflow-hidden">
        <div className="flex items-center justify-between p-4 bg-green-950/50">
          <h4 className="text-green-400 font-bold flex items-center gap-2">
            <Code2 size={20} />
            <span className="font-mono text-sm">// ✅ The Context API Pattern</span>
          </h4>
          <button
            onClick={() => setShowCode(!showCode)}
            className="text-green-300 hover:text-green-100 flex items-center gap-2 text-xs transition-colors"
          >
            {showCode ? <EyeOff size={16} /> : <Eye size={16} />}
            {showCode ? "Hide Code" : "Show Code"}
          </button>
        ...
        {showCode && (
          <div className="border-t border-green-900/50">
            <SyntaxHighlighter code={codeExample} variant="success" />
          </div>
        )}
      </div>
    ...
    <div className="animate-in fade-in duration-500 space-y-6">
      ...
      <div className="bg-gray-900 rounded-2xl shadow-xl overflow-hidden">
        <div className="flex items-center justify-between p-4 bg-red-950/50">
          <h4 className="text-red-400 font-bold flex items-center gap-2">
            <Code2 size={20} />
            <span className="font-mono text-sm">// ❌ Monolithic Context (Everything in One)</span>
          </h4>
          <button
            onClick={() => setShowCode(!showCode)}
            className="text-red-300 hover:text-red-100 flex items-center gap-2 text-xs transition-colors"
    ...
    <div className="animate-in fade-in duration-500 space-y-6">
      ...
      <div className="bg-gray-900 rounded-2xl shadow-xl overflow-hidden">
        <div className="flex items-center justify-between p-4 bg-green-950/50">
          <h4 className="text-green-400 font-bold flex items-center gap-2">
            <Code2 size={20} />
            <span className="font-mono text-sm">// ✅ Split into Logical Contexts</span>
          </h4>
          <button
            onClick={() => setShowCode(!showCode)}
            className="text-green-300 hover:text-green-100 flex items-center gap-2 text-xs transition-colors"
