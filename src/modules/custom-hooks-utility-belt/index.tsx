...
function FixedComponent() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-4 text-slate-300 md:p-8">
      {/* Header */}
      <header className="mb-8 border-b border-yellow-500/30 bg-slate-900/80 backdrop-blur-sm md:mb-12">
        <div className="mx-auto max-w-7xl px-4 py-4 md:px-6 md:py-6">
          {/* First line: Title left, metadata right */}
          <div className="mb-2 flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <Zap className="h-6 w-6 text-yellow-500 md:h-8 md:w-8" />
              <h1 className="text-xl font-bold md:text-3xl">Batman</h1>
            </div>
          ...
          <p className="text-sm font-medium text-yellow-500 md:text-lg">
      ...
      <main className="mx-auto max-w-7xl">
        <div className="grid grid-cols-1 gap-6 md:gap-8 lg:grid-cols-12">
          ...
          <div className="lg:col-span-7">
            <div className="rounded-xl border border-slate-700 bg-slate-900/50 p-6 backdrop-blur-sm md:p-8">
              <div className="mb-6">
              ...
              {chapter === 0 && (
                <div className="mt-8 rounded-lg border border-yellow-500/30 bg-yellow-500/10 p-4">
                  ...
                  <p className="text-slate-300">
                    "The right tool for the right job, ready before the job even
                    begins." Custom hooks encapsulate complex logic into
                    reusable tools.
                  </p>
                </div>
              )}
              {chapter === 1 && (
                <div className="mt-8 rounded-lg border border-red-500/30 bg-red-500/10 p-4">
                  ...
                  <p className="text-slate-300">
                    "Stop building the solution on the fire escape. Build it in
                    the cave." Duplicating stateful logic leads to bugs and
          ...
          <div className="lg:col-span-5">
            <div className="sticky top-8">
              <div className="rounded-xl border border-slate-700 bg-slate-900/80 p-6 backdrop-blur-sm md:p-8">
                <h3 className="mb-6 flex items-center gap-2 text-xl font-bold text-yellow-400">
                  ...
                  <div ref={parent} className="space-y-6">
                    <div className="space-y-4">
                      <h4 className="text-lg font-semibold">
                        Available Gadgets (Custom Hooks)
                      </h4>
                      ...
                      <div className="rounded-lg bg-slate-800/50 p-4">
                        <div className="mb-3 flex items-center justify-between">
                          ...
                          <span
                            className={`rounded px-2 py-1 text-xs ${grapple.isLatched ? "bg-green-500/20 text-green-400" : "bg-slate-700 text-slate-400"}`}
                          >
                            {grapple.isLatched ? "LATCHED" : "READY"}
                          </span>
                        </div>
                        <div className="space-y-3">
                          <div className="relative h-32 overflow-hidden rounded bg-slate-900">
                            {/* Building visualization */}
                            <div className="absolute bottom-0 left-1/2 h-full w-16 -translate-x-1/2 transform bg-gradient-to-b from-slate-700 to-slate-900"></div>
                            {/* Grapple line */}
                            {grapple.isLatched && (
                              <div className="absolute top-0 left-1/2 h-full w-0.5 -translate-x-1/2 transform bg-gradient-to-b from-yellow-500 via-yellow-400 to-yellow-300"></div>
                            )}
                            {/* Batman */}
                            <div
                              className="absolute flex h-8 w-8 items-center justify-center rounded-full border-2 border-yellow-500 bg-slate-800 transition-all duration-300"
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="flex-1 rounded bg-yellow-600 px-4 py-2 text-white transition-colors hover:bg-yellow-700 disabled:cursor-not-allowed disabled:opacity-30"
                            >
                              Fire Grapple
                            </button>
                            <button
                              ...
                              disabled={!grapple.isLatched}
                              className="flex-1 rounded bg-slate-700 px-4 py-2 text-white transition-colors hover:bg-slate-600 disabled:cursor-not-allowed disabled:opacity-30"
                            >
                              Retract
                            </button>
                          </div>
                      ...
                      <div className="rounded-lg bg-slate-800/50 p-4">
                        <div className="mb-3 flex items-center justify-between">
                          ...
                          <span
                            className={`rounded px-2 py-1 text-xs ${smoke.isActive ? "bg-blue-500/20 text-blue-400" : "bg-slate-700 text-slate-400"}`}
                          >
                            {smoke.isActive
                              ? "ACTIVE"
                              : `READY (${smoke.cooldown}s CD)`}
                          </span>
                        </div>
                        <div className="space-y-3">
                          <div
                            className={`flex h-20 items-center justify-center rounded transition-all duration-300 ${smoke.isActive ? "border border-blue-500/50 bg-blue-500/20" : "bg-slate-900"}`}
                          >
                            {smoke.isActive ? (
                          ...
                          <button
                            ...
                            className="w-full rounded bg-blue-600 px-4 py-2 text-white transition-colors hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-30"
                          >
                            {smoke.cooldown > 0
                              ? `Cooldown: ${smoke.cooldown}s`
                              : "Deploy Smoke"}
                          </button>
                        </div>
                      </div>
                    ...
                    <div className="rounded-lg border border-red-500/30 bg-red-500/10 p-4">
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="rounded bg-slate-800/50 p-3">
                            ...
                            <div className="font-mono text-sm">
                              {brokenTrackers.windowSize1.width} ×{" "}
                              {brokenTrackers.windowSize1.height}
                            </div>
                          </div>
                          <div className="rounded bg-slate-800/50 p-3">
                            <div className="mb-1 text-xs text-slate-400">
                            ...
                            <div className="font-mono text-sm">
                              {brokenTrackers.windowSize2.width} ×{" "}
                              {brokenTrackers.windowSize2.height}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="text-sm text-slate-400">
                          <p className="mb-2">
                            ❌ Same logic copied in two places
                          </p>
                ...
                {chapter === 3 && (
                  <div ref={parent} className="space-y-6">
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            onClick={resetDemo}
                            className="flex items-center gap-1 rounded bg-slate-700 px-3 py-1 text-sm transition-colors hover:bg-slate-600"
                          >
                      ...
                      <div className="grid grid-cols-3 gap-4 text-center">
                        <div className="rounded bg-slate-800/50 p-3">
                          ...
                          <div
                            className={`font-semibold ${mode === "broken" ? "text-red-400" : "text-green-400"}`}
                          >
                        ...
                        <div className="rounded bg-slate-800/50 p-3">
                          ...
                          <div
                            className={`font-mono text-lg ${leakedTimers > 10 ? "animate-pulse text-red-400" : "text-slate-300"}`}
                          >
                            {leakedTimers}
                          </div>
                        </div>
                      ...
                      {leakedTimers >= 40 && (
                        <div className="rounded-lg border border-red-500/50 bg-red-500/20 p-3">
                          ...
                          <p className="mt-1 text-sm text-red-300">
                            Auto-reset at 50 leaked timers. This prevents
                            browser crashes.
                          </p>
                        </div>
                      )}
                      ...
                      <div className="flex gap-3">
                        <button
                          ...
                          className="flex flex-1 items-center justify-center gap-2 rounded bg-red-600 px-4 py-2 text-white transition-colors hover:bg-red-700"
                        >
                    ...
                    <CodeBlock
                      code={
                        mode === "broken" ? memoryLeakCode : fixedMemoryLeakCode
                      }
                      ...
                      <div className="space-y-4">
                        <div className="rounded-lg bg-slate-800/50 p-4">
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              ...
                              <span
                                className={`font-medium ${grapple.isLatched ? "text-green-400" : "text-yellow-400"}`}
                              >
                                {grapple.isLatched ? "Latched" : "Ready"}
                              </span>
                            </div>
                        ...
                        <div className="border-t border-slate-700 pt-3">
                          <p className="text-slate-300 italic">
                            "The belt doesn't solve the crime. It handles the
                            repeatable parts so I can."
                          </p>
                        </div>
                      </div>
                    </div>
  ...
  return (
    <div>
      ...
      <div>Window: {windowSize.width} × {windowSize.height}</div>
        ...
        {/* Navigation */}
        <nav className="mt-8 flex justify-between md:mt-12">
          <button
            ...
            <div className="hidden gap-1 md:flex">
              {chapters.map((_, idx) => (
                <button
                  ...
                  className={`h-3 w-3 rounded-full transition-colors ${idx === chapter ? "bg-yellow-500" : "bg-slate-700 hover:bg-slate-600"}`}
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-lg bg-yellow-600 px-4 py-2 text-white transition-colors hover:bg-yellow-700 disabled:cursor-not-allowed disabled:opacity-30 md:px-6 md:py-3"
          >
            Next
