...
<ModuleHeader
  ...
  themeColor="amber"              // Choose from 17 safelisted colors (see Theme & Styling section)
/>
```
...
### ❌ Don't: Use Raw `<pre>` Tags for Code Examples

```tsx
// ❌ Bad: Raw HTML pre tag (no features)
...
```tsx
// ✅ Good: Use CodeBlock component (full-featured)
import { CodeBlock } from "@/components/common/CodeBlock";
...
**Why:** CodeBlock provides essential developer experience features that raw `<pre>` tags don't:
- **Syntax highlighting** - Makes code readable and professional
- **Copy to clipboard** - One-click code copying for users
- **Consistent styling** - Matches platform design across all modules
- **Mobile-friendly** - Proper horizontal scrolling and touch interactions
- **Accessibility** - Proper ARIA labels and keyboard navigation

Raw `<pre>` tags create a subpar learning experience and break visual consistency across the platform.

**When to use CodeBlock:**
- Single code examples
- Inline demonstrations
- Non-comparative code snippets

**When to use CodeComparison:**
...
### ❌ Don't: Use Colors Outside the Safelist
```tsx
// ❌ Bad: Using non-safelisted color
<ModuleHeader themeColor="slate" /> // Won't work! Not in safelist
<ModuleHeader themeColor="stone" /> // Won't work! Not in safelist
```
```tsx
// ✅ Good: Use any of the 17 safelisted colors
<ModuleHeader themeColor="amber" /> // ✓ Works
<ModuleHeader themeColor="orange" /> // ✓ Works
<ModuleHeader themeColor="rose" /> // ✓ Works
<ModuleHeader themeColor="sky" /> // ✓ Works
```

**Why:** Tailwind JIT compilation only includes colors explicitly safelisted in `tailwind.config.ts`. Neutral colors (slate, stone, gray, zinc, neutral) are NOT safelisted for dynamic use.
...
- [ ] Identified appropriate theme color (from 17 safelisted options - see Theme & Styling section)
...
- [ ] Theme color is safelisted (red, orange, amber, yellow, lime, green, emerald, teal, cyan, sky, blue, indigo, violet, purple, fuchsia, pink, rose)
